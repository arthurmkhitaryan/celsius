...
export const aboutApi = createApi({
  ...
  endpoints: (builder) => ({
    getAboutContent: builder.query<About, { locale: string }>({
      query: ({ locale }) => ({
        url: `/about?populate[ourStores][populate]=OurStoresItem.image&populate[ourTeam][populate]=ourTeamItem.image&populate[whoWeAre][populate]=image&locale=${strapiLanguageAdapter(locale)}`,
        headers: {
